...
export default function Register({ setstatus, setName, setPasscode }) {
    ...
    const handleSubmit = async () => {
        if (name.trim() && school.trim() && grade.trim() && passcode.trim()) {
            fetch("/register-student", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({ name, school, grade, passcode })
            })
            .then((res) => {
                if (res.status === 200) {
        ...
        <div className="register-container mt-3 "> 
            ...
            <div className="container mt-3 d-flex justify-content-center">
                <div className="row align-items-center">
                    ...
                    <div className="col-12 col-sm-6"> 
                        <label htmlFor="school" className="form-label"><div className="text-white">School</div></label>
                        <input className="form-control border border-secondary" onChange={(e) => setschool(e.target.value)} value={school} name="school" type="text" />
                    </div>
                    <div className="col-12 col-sm-6"> 
                        <label htmlFor="grade" className="form-label"><div className="text-white">Grade</div></label>
                        <input className="form-control border border-secondary" onChange={(e) => setgrade(e.target.value)} value={grade} name="grade" type="number" min="8" max="12" />
                    </div>
                    <div className="col-12 col-sm-6"> 
